...
export function PengeluaranBarangForm({ onSuccess, onCancel }: PengeluaranBarangFormProps) {
  ...
  const fetchApprovedSRs = async () => {
    try {
      const response = await fetch("/api/pt-pks/gudang/pengeluaran-barang/approved-sr");
      if (response.ok) {
  ...
  const handleSubmit = async (srId: string) => {
    setSubmitting(true);
    try {
      const response = await fetch("/api/pt-pks/gudang/pengeluaran-barang/create-from-sr", {
        method: "POST",
        headers: {
                        ...
                        {sr.items.map((item) => {
                          ...
                          return (
                            <TableRow key={item.id} className={!isStockSufficient ? "bg-red-50" : ""}>
                              ...
                              <TableCell>{item.material.kategoriMaterial.nama}</TableCell>

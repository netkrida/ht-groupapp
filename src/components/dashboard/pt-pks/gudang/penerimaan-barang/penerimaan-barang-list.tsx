  ...
  const fetchPenerimaanBarang = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/pt-pks/gudang/penerimaan-barang?${params.toString()}`
      );
      if (response.ok) {
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => router.push('/dashboard/gudang/penerimaan-barang/create')}>
                ...
                {filteredPenerimaan.length === 0 ? (
                  ...
                  filteredPenerimaan.map((penerimaan) => (
                    <TableRow key={penerimaan.id}>
                      ...
                      <TableCell>{penerimaan.purchaseOrder.nomorPO}</TableCell>
                      <TableCell>{penerimaan.purchaseOrder.namaSupplier}</TableCell>
